...
exports.validateTx = async (txid) => {
  try {
    if(process.env.ETHERSCAN == "true") {
        const r = await nodeFetch(`https://api.etherscan.io/api?module=transaction&action=gettxreceiptstatus&txhash=${txid}&apikey=${process.env.ETHERSCAN_API}`).then(res => res.json())
